	...
	<cffunction name="moveToNextSibling" output="false" returntype="struct" hint="Moves the node 'src' and all its children (subtree) so that it is the next sibling of 'dst'.">
		...
		<cflock name="#getLockName()#" type="exclusive" timeout="#getLockTimeout()#">
			...
			<cfif not isValidNode(src)>
				<cfthrow type="InvalidNode" message="MoveToNextSibling: The ""src"" node (#nodeToString(src)#) is invalid.">
			</cfif>
			<cfset dst = loadNode(arguments._dst)>
			<cfif not isValidNode(dst)>
				<cfthrow type="InvalidNode" message="MoveToNextSibling: The ""dst"" node (#nodeToString(dst)#) is invalid.">
			</cfif>
			<cfif isChild(dst,src)>
				<cfthrow type="InvalidMove" message="MoveToNextSibling: The ""src"" node #nodeToString(src)# cannot be moved into its subtree at node ""dst"" #nodeToString(arguments.dst)#.">
			</cfif>
